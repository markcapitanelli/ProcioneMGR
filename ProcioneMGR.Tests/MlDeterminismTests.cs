...
public class MlDeterminismTests
{
    ...
    private sealed class FixedChampionRegistry(SavedMlModel model) : IModelRegistry
    {
        ...
        public Task<IReadOnlyList<SavedMlModel>> ListGroupAsync(string symbol, string timeframe, CancellationToken ct = default)
            => throw new NotSupportedException();
        public Task<StageChangeOutcome> PromoteToChallengerAsync(int modelId, CancellationToken ct = default) => throw new NotSupportedException();
        ...
        public Task<StageChangeOutcome> RetireAsync(int modelId, string reason, bool requestRetrain, CancellationToken ct = default) => throw new NotSupportedException();
