...
public class BayesianOptimizerTests
{
    ...
    [Fact]
    public async Task Maximize_1D_ConvergesNearOptimum()
    {
        ...
        var result = await NewSearch(seed: 1).MaximizeAsync(space, p => Task.FromResult(Objective(p)), iterations: 30, initialRandom: 8, seed: 1);
    ...
    [Fact]
    public async Task Maximize_2D_ConvergesNearOptimum()
    {
        ...
        var result = await NewSearch(seed: 3).MaximizeAsync(space, p => Task.FromResult(Objective(p)), iterations: 45, initialRandom: 12, seed: 3);
    ...
    [Fact]
    public async Task Maximize_IsDeterministic_ForSameSeed()
    {
        var space = new ParameterSpace([new ParameterDimension("x", 0, 5)]);
        double Objective(double[] p) => -Math.Pow(p[0] - 3.0, 2);

        var a = await NewSearch().MaximizeAsync(space, p => Task.FromResult(Objective(p)), iterations: 20, initialRandom: 5, seed: 7);
        var b = await NewSearch().MaximizeAsync(space, p => Task.FromResult(Objective(p)), iterations: 20, initialRandom: 5, seed: 7);
    ...
    [Fact]
    public async Task Maximize_BeatsPureRandomSearch_OnSameBudget()
    {
        ...
        var guided = await NewSearch(seed: 5).MaximizeAsync(space, p => Task.FromResult(Objective(p)), iterations: 30, initialRandom: 5, seed: 5);
