...
| C4.a | **Nucleo di etichettatura FATTO 2026-07-27**: `TripleBarrierLabeler` (tre barriere, barriere derivate dalle escursioni reali via `ExcursionAnalyzer`, pesi per unicità media AFML §4.3) + `MetaLabeler` (filtro dei falsi positivi con verdetto che pretende ANCHE un campione superstite non ridicolo). 23 test, incluso l'**edge piantato** che la catena recupera. Due onestà cablate: l'ambiguità intra-barra si risolve sempre a favore dello stop (lettura pessimistica), e le barre in coda restano senza etichetta invece di riceverne una troncata | fatto — nucleo | il consumo (decoratore di `IStrategy`, sizing per probabilità, barre informative M2) resta aperto: il nucleo è misurabile da solo e va provato su un edge che abbia già passato i gate, mai su una regola morta |
