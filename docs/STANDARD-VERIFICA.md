...
| **Fase B** sentiment LLM (2026-08-01) | ✅ parsing difensivo (numero/virgola/testo attorno/clamp), array di batch che PRETENDE la lunghezza esatta (disallineato = fallimento del batch, mai assegnazione a scalare); guard VERO nei test, finto solo il client | ✅ ogni esito non-Ok (chiave assente, breaker, eccezione, risposta garbage) ⇒ fallback al lessico e MAI un'eccezione al chiamante; elemento non scorabile ⇒ SALTATO e ritentato (mai uno zero inventato) | ✅ `AltDataSyncServiceTests`/`SentimentScorerComparisonServiceTests` su Postgres vero: scorer che lancia non ferma la sync; LLM interamente ripiegato ⇒ riga «non disponibile», non un duplicato travestito | ✅ /sentiment sull'app vera: scorer LLM con chiave NVIDIA reale, confronto con punteggi reali (sotto) |
| **Fase C** secondo parere (2026-08-01) | ✅ Kind distinto + provider nello StageName; ModelUsed del client di confronto | ✅ fallimento del confronto NON tocca la primaria; provider coincidente/ignoto/senza chiave ⇒ skip a voce; primaria in errore ⇒ nessun confronto | ✅ `PipelineSupervisorComparisonTests` su Postgres vero, inclusa l'anti-join del worker che IGNORA i Kind di confronto | ✅ /admin/ai-supervisor sull'app vera: toggle salvato e riquadro «Secondo parere» (sotto) |
| **Pilota ONNX** L1 (2026-08-01) | ✅ PARITÀ ML.NET↔ONNX Runtime ≤1e-3 attraverso lo scorer reale (il riferimento indipendente è il framework che ha addestrato); FNV-1a ancorato al valore noto; determinismo del vettorizzatore | ✅ direzione conservata su frasi MAI viste; **due difetti trovati dai test**: corpus con scorciatoie degeneri (token unici per riga ⇒ probe a zero) e causa di scarto fotografata DOPO il delete (diceva sempre «non si carica») | ✅ `OnnxSentimentPilotTests` su Postgres vero: treno→esporta→carica→parità end-to-end; troppo poche righe ⇒ rifiuto onesto; modello mancante ⇒ fallback | ✅ addestramento dal pannello /sentiment sull'app vera (sotto) |

| **Filone E** — bonifica «controlli che rassicurano» (2026-07-31) | ⚠️ parziale per natura (impianto, non matematica), con l'analogo giusto coperto: i default di fabbrica passano TUTTE le regole di validazione, inclusa quella NUOVA per `Trading:Safety` (prima non esisteva: il canale gRPC avrebbe accettato qualunque payload) | ✅ adattato come per l'audit: «il rumore non accende niente» = nessun falso allarme — il replay di avvio (battito vecchio ma che AVANZA) non fa scattare l'inedia; serie sane su 5 giri ⇒ zero allarmi; Disabled/RateLimited NON contano come guasti del canale | ✅ watchdog + battito su Postgres vero e motore VERO (`ProcessCandle` ⇒ battito esposto dallo status); guardia di freschezza su Postgres vero coi casi-trappola (serie vuota ≠ aggiornata; N ferme insieme ⇒ UN messaggio); store del motore finto condiviso per i pannelli | ✅ eseguito la sera stessa sull'app vera col motore nuovo in-cluster: soglia cambiata dal pannello e **verificata dentro il pod** (19 campi, inclusi i 2 B1), battito accanto a RUNNING su 1d e 15m (0 barre indietro), «Prova dal motore» CONSEGNATA con spia aggiornata via rpc, 221 serie «aggiornata» e le 7 disabilitate «non sincronizzata» |
| **Audit backend↔frontend** (2026-07-29/30) | ⚠️ **parziale, e per una ragione**: qui non c'è matematica contro cui confrontarsi — è impianto e politica. L'analogo più vicino c'è: i default di fabbrica devono passare **tutte** le regole di validazione (una regola che rifiuta la configurazione di fabbrica è un bug della regola), e la tabella di verità di `ShouldAlertStale` è coperta caso per caso | ⚠️ **adattato**: «il rumore non accende niente» diventa «nessun falso allarme», coperto sui due percorsi di allerta toccati (grazia della staleness; nessun warning di override quando il file è l'unica sorgente). Non applicabile al resto — nessuno di questi strumenti emette un verdetto statistico | ✅ `EngineConfigGrpcTests`: endpoint serviti dall'host **reale** `ProcioneMGR.Trading` su HTTP/2, coi rifiuti distinguibili sul filo (PermissionDenied ≠ InvalidArgument) e la prova che i segreti non escono nemmeno se richiesti esplicitamente. Più `TradingServiceCollectionExtensionsTests`: locale/remoto scelti **per costruzione**. *Aggiunti rileggendo questo documento DOPO aver dichiarato il lavoro finito: mancavano, ed è la regola 1* | ✅ il livello che ha trovato **tutto**: sette difetti, nessuno visibile ai test. Config letta dal motore via gRPC, scrittura verificata **dentro il pod**, interruttore del feed provato nei due versi sul core in esercizio, notifica del motore `Delivered` con **conferma di ricezione dal proprietario** |
...
Il livello 4 non ha trovato *qualche* difetto: ha trovato **tutti** quelli di quella sessione, sette
su sette, e nessuno era visibile ai livelli 1-3. Il motivo è che condividevano una forma sola:
...
caso per cui era stato scritto. E il settimo, il più subdolo: la scheda Notifiche che diceva
«abilitale qui e salva, si scrive sul motore» mentre scriveva sul canale del **guscio** — seguendo
le istruzioni a schermo si vedeva un messaggio verde e il motore restava muto. Un test unitario non
può vedere niente di tutto questo: gira su configurazioni sintetiche, mentre il guasto vive **fra
due processi**, o fra l'interfaccia e la realtà.
