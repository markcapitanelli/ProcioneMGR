...
| 4c | Sentiment 2.0 | `Sentiment` | Enable (default **ON**: le serie Binance pubbliche esistono solo 30 giorni — spento = buchi irrecuperabili nei baseline), cadenze metriche/news ⟳, simboli CSV, soglia z estremi, soglie F&G, **Feature ML ⚠️** (opt-in esplicito: cambia cosa vedono i modelli); bottone "Esegui ora". [2026-08-13] Sotto-blocco **Guardiano serie-patrimonio** (`Sentiment:HeritageGuard`): enable (default ON), cadenza ⟳, simboli funding CSV (vuoto = i sei del backfill T0.2), data-àncora + punti minimi per funding/F&G/liquidazioni — le soglie contro cui il guardiano misura la profondità (il funding dal 2019 è sparito due volte in silenzio), più l'interruttore **«Sorveglia liquidazioni»** (OFF = riga misurata ma «NON SORVEGLIATA»: dalle postazioni EEA lo stream futures è muto per MiCA e l'allarme sarebbe perpetuo); regole server-side: intervallo ≥ 1h, soglie ≥ 1, date nel passato. Àncora funding di default 2020-10-01: una serie non può precedere il listing del suo mercato (SOL è il più tardo, 2020-09-13) |
